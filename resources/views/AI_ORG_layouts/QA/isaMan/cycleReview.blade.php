      ...
      <table class="ui celled table datatable" id="cycleReviewTable">
        ...
        @foreach($ROBS as $document)
            <tr>
                ...
                @if($document->leadassessor != 1)
                    @if($document->qa == 0)
                      <i class="checkmark box large green icon link validateApprover" data-doc="{{$document->id}}" data-user="qa"></i>
                    @else
                      <i class="remove large red icon link unValidateApprover" data-doc="{{$document->id}}" data-user="qa"></i>
                    @endif
                @else
                  Validated!
                @endif
