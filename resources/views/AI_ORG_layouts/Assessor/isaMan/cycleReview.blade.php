      ...
      <table class="ui celled table datatable" id="cycleReviewTable">
        ...
        @foreach($ROBS as $document)
            <tr>
                ...
                <td>
                @if($document->leadassessor != 1)
                  @if($document->approver > 0 && $document->qa > 0 && $document->projectmanager > 0)
                    @if($document->assessor == 0)
                      <i class="checkmark box large green icon link validateApprover" data-doc="{{$document->id}}" data-user="assessor"></i>
                    @else
                      <i class="remove large red icon link unValidateApprover" data-doc="{{$document->id}}" data-user="assessor"></i>
                    @endif
                  @else
